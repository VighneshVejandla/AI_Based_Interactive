...
class PDFProcessor:
    ...
    def save_to_excel(self, tables, output_path):
        """
        Save the extracted tables to an Excel file after cleaning currency symbols.
        """
        try:
            with pd.ExcelWriter(output_path) as writer:
                for i, table in enumerate(tables):
                    # Clean currency symbols in the DataFrame
                    table = table.applymap(self.clean_currency)
                    table.to_excel(writer, sheet_name=f"Sheet{i+1}", index=False)
            print(f"Tables saved to {output_path}")
        except Exception as e:
            print(f"Error saving to Excel: {e}")